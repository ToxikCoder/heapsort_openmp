...
//1)Получить количество потоков
//2)Разбить массив на количество потоков
//3)Отсортировать каждый подмассив параллельно
//4)Соединить слиянием
...
void awesomeGenerator(char* file, int num)
{
	...
	for (i = 0; i < num; ++i)
		*ofs << rand() % 1000000 + 1 << endl;

	auto after = static_cast<float>(clock()) / CLOCKS_PER_SEC;
	cout << "Сгенерирован новый файл на " << num << " чисел за " << after - before << endl;
	ofs->close();
	delete ofs;
}
...
int main(int argc, char argv[])
{
	...
	cout << "Время сортировки " << arr.size() << " элементов последовательно: " << posl << endl;

	before = static_cast<float>(clock()) / CLOCKS_PER_SEC;
	heap_sort_parallel(arr_par);
	after = static_cast<float>(clock()) / CLOCKS_PER_SEC;
	auto par = after - before;
	cout << "Время сортировки " << arr.size() << " элементов параллельно: " << par << endl;
	cout << "Преимущество: " << 100 - (par / posl * 100) << "%" << endl;

	auto equal = true;
	for (auto i = 0; i < arr.size(); ++i)
	{
		equal = equal && (arr.at(i) == arr_par.at(i));
		if (!equal)
			break;
	}
	
	equal ? cout << "Отсортированные массивы равны. Сортировка успешна" << endl : cout << "Отсортированные массивы не равны. Сортировка провалилась" << endl;
	arr.clear();
	arr_par.clear();
	system("pause");
    return 0;
